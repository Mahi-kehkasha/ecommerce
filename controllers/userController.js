...
const loadSingleProduct = async (req, res) => {
  try {
    ...
    const products = await Book.findById(productID).populate("category");

    if (!products) {
      return res.render("single-product", { error: "Product not found" }); // Corrected the view name to "single-product"
    }

    res.render("single-product", { products: products,userId:userId }); // Corrected the view name to "single-product"
  } catch (error) {
    console.log(error.message);
    res.render("single-product", { error: "Server error" }); // Corrected the view name to "single-product"
  }
};
...
const getLatestArrivals = async (req, res) => {
  ...
  try {
    const latestArrivals = await Product.find({ arrivalDate: { $gte: cutoffDate } })
      .sort({ arrivalDate: -1 })
      .limit(10); // You can limit the number of products returned
...
// Controller function for filtering products based on price range
 const filterProducts = (req, res) => {
  const minPrice = parseFloat(req.query.minPrice);
  const maxPrice = parseFloat(req.query.maxPrice);
  console.log(minPrice,"minnnnnnnn");

  // Assuming you have a function to fetch products from the database
  // You can filter the products based on the provided price range
...
const handleSearchByLang = async (req, res) => {
  try {
    ...
    const books = await Book.find({ languages: languages })
      .populate("category")
      .sort({ addedDate: -1 })
      .limit(limit * 1)
      .skip((page - 1) * limit);
    ...
    if (paymentMethod === "Razorpay") {
      ...
      paypal.payment.create(paypalPaymentData, (error, payment) => {
        if (error) {
          ...
          return res
            .status(500)
            .json({ success: false, error: "PayPal payment creation failed" });
        } else {
...
const myAccount = async (req, res) => {
  ...
  const userId = req.session.user; // Assuming you are using sessions to store user data

  const userdetails = await User.findOne({_id:userId})
  const addresses = await Address.find({ user: userId });

  res.render('my-account', { user: userId ,userdata : userdetails, addresses:addresses});
...
const viewOrders = async (req, res) => {
  try {
    ...
    res.render("view-orders", { orders: separateOrders,totalPages: Math.ceil(count / limit),
    currentPage: page ? page : 1,userId });
  } catch (error) {
    console.log(error.message);
  }
};
...
console.log(req.body,"KKKKKKGDFGGDSDDSSSSSSSS");
  // Update the user's details in the database
  User.findByIdAndUpdate(
    req.session.user, // Assuming the authenticated user's ID is stored in req.session.user
...
const applyCoupon = async(req, res) => {
  try {
    const { couponCode, bookId } = req.body;
    ...
    const discountedPrice = book.price * (1 - coupon.percentage / 100);
...
// .......................
const viewWishlistItems = async (req, res) => {
  try {
    const userId = req.session.user;
    ...
    if (!wishlist) {
      // If the wishlist doesn't exist, you can handle it appropriately (e.g., show an empty wishlist page)
      return res.render("wishlist", { wishlist: [] });
    }

    // Fetch the book details for the items in the wishlist
    const wishlistItems = wishlist.items

    res.render("wishlist", { wishlist: wishlistItems , userId });
  } catch (error) {
    console.error("Error viewing wishlist items:", error);
    return res.status(500).render("failure", { error: "Failed to view wishlist items" });
  }
};
...
const addToWishlist = async (req, res) => {
  try {
    ...
    console.log(req.body, "wishlist req");
